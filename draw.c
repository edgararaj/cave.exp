...
void print_ellipse(WINDOW *win, Vec2i win_size, int x, int y, int r2, int r1) {
    ...
    for (int i = min_y; i < max_y; i++) {
        ...
        for (int j = min_x; j < max_x; j++) {
            if (sqrt(pow(i - y - r1, 2) / pow(r1, 2) +
                     pow(j - x - r2, 2) / pow(r2, 2)) > 1)
                continue;
            mvwprintw(win, j, i, "?.");
        }
    }
...
void print_rectangle(WINDOW *win, Rect rect) {
    ...
    print_rectangleu(win, rect.tl.y, rect.tl.x, rect.br.y - rect.tl.y,
                     rect.br.x - rect.tl.x);
}

void print_rectangleu(WINDOW *win, int startrow, int startcol, int height,
                      int width) {
    for (int r = startrow; r <= startrow + height; r++) {
...
void print_line(WINDOW *win, Line line) {
    ...
    float delta_y = (float)(line.end.y - line.start.y) /
