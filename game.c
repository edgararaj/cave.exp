...
void init_game(GameState *gs, Rect window)
{
    ...
    if (gs->illuminated.data)
    {
        free(gs->illuminated.data);
    }
...
{
    Vec2i pos = vec2f_to_i(
        rect_center(rect_translate(project_rect(gs->camera, rect_float_to_rect(arrow.rect)), (Vec2i){-1, -1})));
